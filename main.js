...
function findPokemons(criteria) {
  // const jenisType = [`fire`, `grass`, `water`, `normal`, `electric`];
  // const jenisHp = [35, 40, 45, 50, 55, 60];
  // const jenisAttack = [45, 50, 55, 60, 65, 70];
  // const jenisDefense = [45, 50, 55, 60, 65];
  // const jenisIsFlying = [true, false];

  const result = [];
  //mencari by criteria
  //kalo perulangan biasa for
  /*for(let i = 0; i < pokemons.length; i++) {
    const pokemon = pokemons[i];
    const{data} = pokemon;*/
  //kalo perulangan for forEach
  /*pokemons.forEach(function(pokemon) {
    const { data } = pokemon;*/
  for(const pokemon of pokemons){
    const{data} = pokemon;
    //type,hp,attack,defense dan isFlying
    switch(criteria.searchBy){
      //1
      case `type`:
        if (data.type === criteria.value) {
          result.push(pokemon.name);
        }
        break;
      //2
      case `hp`:
        if (data.hp === criteria.value) {
          result.push(pokemon.name);
        }
        break;
      //3
      case `attack`:
        if (data.attack === criteria.value) {
          result.push(pokemon.name);
        }
        break;
      ...
      //4
      case `defense`:
        if (data.defense === criteria.value) {
          result.push(pokemon.name);
        }
        break;
      ...
      //5
      case `isFlying`:
        if (data.isFlying === criteria.value) {
          result.push(pokemon.name);
        }
        break;
    }
